    ...
    *[Symbol.iterator]() {
    ...
    static getDefinition(meta, orderBy) {
        return Utils_1.Utils.asArray(orderBy).flatMap(order => {
            return Utils_1.Utils.keys(order)
                .map(key => meta.properties[key])
                .filter(prop => prop && ([enums_1.ReferenceKind.SCALAR, enums_1.ReferenceKind.EMBEDDED, enums_1.ReferenceKind.MANY_TO_ONE].includes(prop.kind) || (prop.kind === enums_1.ReferenceKind.ONE_TO_ONE && prop.owner)))
                .map(prop => [prop.name, order[prop.name]]);
        });
    }
