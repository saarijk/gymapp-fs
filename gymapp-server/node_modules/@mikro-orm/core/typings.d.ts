...
export type EntityDTOProp<E, T, C extends TypeConfig = never> = T extends Scalar ? T : T extends LoadedReference<infer U> ? EntityDTONested<U, C> : T extends Reference<infer U> ? PrimaryOrObject<E, U, C> : T extends ScalarReference<infer U> ? U : T extends {
    getItems(check?: boolean): infer U;
} ? (U extends readonly (infer V)[] ? EntityDTONested<V, C>[] : EntityDTONested<U, C>) : T extends {
    $: infer U;
} ? (U extends readonly (infer V)[] ? EntityDTONested<V, C>[] : EntityDTONested<U, C>) : T extends readonly (infer U)[] ? (T extends readonly [infer U, ...infer V] ? T : U[]) : T extends Relation<T> ? EntityDTONested<T, C> : T;
export type EntityDTO<T, C extends TypeConfig = never> = {
    [K in EntityKey<T> as ExcludeHidden<T, K>]: EntityDTOProp<T, T[K], C>;
};
...
type LoadedLoadable<T, E extends object> = T extends Collection<any, any> ? LoadedCollection<E> : T extends Reference<any> ? LoadedReference<E> : T extends ScalarReference<infer U> ? LoadedScalarReference<U> : T extends Scalar | Scalar[] ? T : E;
